...
public class MiniMaxGameImproved {
    ...
    public int staticEvaluationMidEndImproved(char[] board){
        int w = 0, b = 0;
        ...
        if(b <= 2)
            ...
            return((1000*(cntMills(board, true)-cntMills(board, false)))-list.size());
    }

    public int cntMills(char[] board, boolean player){
        int mill = 0;
        char p = Utils.whichPlayer(player);
    ...
    public char[] maxMin(char[] board, int ply) {
        if(ply == 0) {
            ...
            for(char[] child : childNodes) {
                ...
                if(evaluation < staticEvaluationMidEndImproved(minChoice)) {
                    evaluation = staticEvaluationMidEndImproved(minChoice);
    ...
    public char[] minMax(char[] board, int ply) {
        if(ply == 0)
            ...
            for(char[] child : childNodes) {
                ...
                if(evaluation > staticEvaluationMidEndImproved(maxChoice)) {
                    evaluation = staticEvaluationMidEndImproved(maxChoice);
                    selectedMin = child;
                }
            }
            return selectedMin;
